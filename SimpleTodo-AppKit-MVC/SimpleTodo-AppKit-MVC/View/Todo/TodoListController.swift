...
class TodoListController: NSViewController {
    ...
    var items: [Todo] {
        return repository.selectedSubject?.todos ?? []
    }
...
//        titleColumn.width = 180
    ...
    @objc func addRow() {
        guard repository.subjectSelected else { return }
        repository.addNewTodo(withTitle: "New Job")
    ...
    @objc func deleteSelectedRows() {
        ...
        for index in sortedIndexes {
            repository.removeTodo(at: index)
        }
...
extension TodoListController: NSTableViewDataSource {
...
extension TodoListController: NSTableViewDelegate {
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        switch id.rawValue {
        case "title":
            cell.textField?.stringValue = todo.title
        default:
            break
        }
    ...
    func tableViewSelectionDidChange(_ notification: Notification) {
        ...
        if selectedIndexes.count > 0 {
            ...
            repository.selectTodo(at: selectedRow)
        } else {
            repository.deselectTodo()
        }
        homeController.reloadTodoDetail()
    }

}

extension TodoListController: NSTextFieldDelegate {
    ...
    func controlTextDidEndEditing(_ notification: Notification) {
        ...
        case "title":
            let todo = items[row]
            todo.title = textField.stringValue
        default:
            break
        }
